...
MODEL_DIR = os.path.join(BASE_DIR, "models", "GRU")  # Store trained GRU models
...
def prepare_data(df, target_column="Close", sequence_length=60, test_size=0.2):
    ...
    return X_train, y_train, X_test, y_test, scaler_features, scaler_target, df.index[split_idx + sequence_length]
...
# Function to Train and Save GRU Model
def custom_gru_model(stock, filepath):
    print(f"Training GRU model for {stock}...")

    # Load data
    df = load_stock_data(filepath)
    X_train, y_train, X_test, y_test, scaler_features, scaler_target, prediction_start_date = prepare_data(df)
    ...
    # Save trained model
    model_path = os.path.join(MODEL_DIR, f"{stock}_gru.h5")
    ...
    metrics_path = os.path.join(METRICS_DIR, f"{stock}_gru_metrics.csv")
    ...
    predictions_path = os.path.join(PREDICTIONS_DIR, f"{stock}_gru_results.csv")
...
# Loop Through All Stocks and Train GRU Models
for stock, filename in TICKERS.items():
    filepath = os.path.join(DATA_DIR, filename)
    custom_gru_model(stock, filepath)

print("All GRU models have been trained and saved.")
